...
PULSE_TS = r'''export type PulseD1Statement = {
...
Path('prototype/vanilla/pulse.ts').write_text(PULSE_TS)
...
replace('prototype/vanilla/worker.ts', '<section class="toolbar">',
...
replace('prototype/vanilla/public/benchmark.js', "const sourceError = $('#source-error');", "const sourceError = $('#source-error');\nconst pulseEl = $('#pulse');")
...
replace('prototype/vanilla/public/benchmark.js', 'setInterval(()=>loadCameras(false),5*60*1000);', 'setInterval(()=>loadCameras(false),5*60*1000);\nsetInterval(()=>loadPulse(false),PULSE_REFRESH_MS);')
replace('prototype/vanilla/public/benchmark.js',
...
'''
with Path('prototype/vanilla/public/benchmark.css').open('a') as f:
    f.write(PULSE_CSS)

print('Seattle Pulse patch applied')
