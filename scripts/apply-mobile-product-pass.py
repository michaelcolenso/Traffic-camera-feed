...
simple = {
    'className={`rounded-xl border p-2 transition ${showSettings ?': 'className={`flex h-11 w-11 items-center justify-center rounded-xl border transition ${showSettings ?',
    '<main className="mx-auto max-w-7xl px-4 pb-24 pt-6 md:py-8">': '<main className="mx-auto max-w-7xl px-3 pb-28 pt-4 sm:px-4 sm:pt-6 md:py-8">',
    '<div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">': '<div className="grid grid-cols-1 gap-4 sm:grid-cols-2 sm:gap-5 lg:grid-cols-3 xl:grid-cols-4">',
}
for old, new in simple.items():
    if old not in text:
        raise SystemExit(f'Expected simple source not found: {old!r}')
    text = text.replace(old, new, 1)
